-- FIX USERNAME UNIQUE AND EMAIL VERIFICATION
...
-- Step 7: Create a function to force email verification (for testing)
CREATE OR REPLACE FUNCTION public.force_verify_email(user_email TEXT)
RETURNS BOOLEAN AS $$
BEGIN
  UPDATE auth.users 
...
COMMENT ON FUNCTION public.force_verify_email(TEXT) IS 'Forces email verification (for testing)';

-- Step 9: Show current usernames to check for duplicates
SELECT 
  username,
  COUNT(*) as count
...
-- Step 10: Show current users and their verification status
SELECT 
  u.id,
  u.email,
...
--
-- To check email verification status:
-- SELECT * FROM public.get_email_verification_status('user@example.com');
--
-- To force verify an email (for testing):
-- SELECT public.force_verify_email('user@example.com'); 